...
- **零配置代理** — 插件在 dsh web 内部注册 `/ds-usage` 路由,浏览器只访问本机地址
...
```
浏览器 (React overlay)
   │  fetch /ds-usage            (每 60s,本机 loopback)
   ▼
dsh web 宿主进程 (插件宿主端)
...
- **宿主端**(`lib/index.js`):注册 `/ds-usage` 路由,带 60 秒缓存与 10 秒超时;密钥只存在于宿主进程内存中
...
1. 将本仓库 `ds-usage` 包目录复制到 `%USERPROFILE%\.dsh\profiles\web\node_modules\ds-usage\`
...
5. 侧边栏出现 **余额 ¥xx.xx** 即成功;也可直接访问 `http://127.0.0.1:3080/ds-usage` 查看接口返回
...
| 现象 | 原因与处理 |
|---|---|
| 显示"余额不可用" | API Key 缺失、无效或账户不可用。检查 `.credentials.yaml`;直接访问 `/ds-usage` 看返回的 `error`(`missing-api-key` / `http-401`) |
