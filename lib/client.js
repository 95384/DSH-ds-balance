...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const css = [
      ...
      '.hHd-Xa_collapsed .ds-balance-box{height:36px;width:36px;margin:0 0 12px;padding:0;border-color:transparent;background:transparent}',
      // Collapsed hover highlight — same treatment as the New Session button.
      '.hHd-Xa_collapsed .ds-balance-box:hover{background:var(--dsw-alias-interactive-bg-hover)}',
    ...
    function BalanceBox() {
      ...
      // Collapsed flag, used only for the hover tooltip (the layout itself
      // is driven purely by CSS from the shell's .hHd-Xa_collapsed class).
      react.useEffect(() => {
        const frame = document.querySelector('[data-shell-overlay]')?.parentElement ?? null;
        if (!frame) return;
        const update = () => setCollapsed(frame.hasAttribute('data-sidebar-collapsed'));
        update();
      ...
      return reactDom.createPortal(
        react.createElement(
          'div',
          {
            ...
            // Tooltip only in the collapsed rail (no "every 60s" note).
            title: collapsed ? 'DeepSeek 账户余额: ' + balanceText : undefined,
          },
          react.createElement(
            react.Fragment,
            null,
