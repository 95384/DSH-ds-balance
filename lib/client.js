/**
 * ds-usage — client half (hand-written module bundle).
 *
 ...
 * proxy route /ds-usage every 60s; hides while the sidebar is collapsed.
 *
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // ── styles ─────────────────────────────────────────────────────────────
    const css = [
      '.ds-usage-box{position:fixed;left:0;box-sizing:border-box;display:flex;align-items:center;justify-content:center;gap:6px;padding:0 12px;height:28px;border:1px solid var(--dsw-alias-border-l2);border-radius:8px;background:var(--dsw-alias-bg-module-platform);color:var(--dsw-alias-label-primary);font-size:12px;line-height:1;white-space:nowrap;overflow:hidden;cursor:default;z-index:20;user-select:none}',
      '.ds-usage-box .ds-usage-label{color:var(--dsw-alias-label-secondary)}',
      '.ds-usage-box .ds-usage-value{font-weight:600;font-variant-numeric:tabular-nums}',
      '.ds-usage-box .ds-usage-err{color:var(--dsw-alias-label-secondary);font-size:11px}',
    ...
    const tagId = 'ds-usage/styles.css';
    ...
    function BalanceBox() {
      ...
      react.useEffect(() => {
        ...
        const load = async () => {
          try {
            const res = await fetch('/ds-usage', { cache: 'no-store' });
      ...
      if (data.status === 'ok' && data.body && data.body.ok) {
        inner = react.createElement(
          react.Fragment,
          null,
          react.createElement('span', { className: 'ds-usage-label' }, '余额'),
          react.createElement('span', { className: 'ds-usage-value' }, formatBalance(data.body.balance)),
        );
      } else if (data.status === 'ok') {
        inner = react.createElement('span', { className: 'ds-usage-err' }, '余额不可用');
      } else if (data.status === 'error') {
        inner = react.createElement('span', { className: 'ds-usage-err' }, '余额获取失败');
      } else {
        inner = react.createElement('span', { className: 'ds-usage-err' }, '余额 …');
      }

      return react.createElement(
        'div',
        {
          className: 'ds-usage-box',
    ...
    function apply(ctx) {
      ctx.effect(
        () => ctx.slots.register({ name: 'shell.overlay', id: 'ds-usage' }, BalanceBox),
        'ds-usage: balance box',
      );
    }
    exports.apply = apply;
