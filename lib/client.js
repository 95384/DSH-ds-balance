  ...
  factory: (require) => {
    ...
    function BalanceBox() {
      ...
      react.useEffect(() => {
        ...
        // The sidebar width animates via a grid-template-columns transition; the
        // observer above fires at the transition start (stale width), so re-measure
        // once the transition actually finishes.
        const onTransitionEnd = (ev) => {
          if (ev.propertyName === 'grid-template-columns') update();
        };
